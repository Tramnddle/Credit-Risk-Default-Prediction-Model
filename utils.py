...
def score_distribution(target_list, pred_list, data_type_list):
    for i in range(len(data_type_list)):
        ...
        for label, color in zip([1, 0], ['red', 'green']):
            data = sub_df[sub_df['y_actual'] == label]['y_predicted']
            density = gaussian_kde(data)
            xs = np.linspace(0, 1, 200)
            plt.plot(xs, density(xs), label="Defaulter" if label == 1 else "Non-Defaulter", color=color)

        plt.xlabel('Predicted positive class score')
        plt.ylabel('Density')
        plt.title(f'{df_type} Distribution of predicted score')
...
def shap_importance(model, data_list, data_type_list):
    """
    Plot SHAP for top 20 features per dataset (Training, Validation, Hold Out)
    """
    import shap
    import matplotlib.pyplot as plt
    ...
    for i, d in enumerate(data_list):
        features = d[model.feature_name()]
        shap_values = explainer.shap_values(features)

        # Binary classification — use shap_values[1] for class 1
        if isinstance(shap_values, list):
            shap_to_plot = shap_values[1]
        else:
            shap_to_plot = shap_values

        # Let SHAP fully handle the plot (fixes colorbar issue)
        shap.summary_plot(
            shap_to_plot,
            features,
            plot_type="bar",
            max_display=20,
            show=True,           # Don't suppress plot rendering
            plot_size=(14.5, 10.5)
        )

        # You can optionally add a title after the SHAP plot
        plt.title(f"{data_type_list[i]} SHAP Values", fontsize=18)
        plt.tight_layout()
        plt.show()
